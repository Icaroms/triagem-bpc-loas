 ...
 *
 * Nesta versão passou a hospedar o painel de backup, já que é a aba onde o
 * usuário lida diretamente com o acervo de dados.
 */
...
export default function TabHistorico({
  ...
  const filtered = history.filter((h) => {
    ...
    return !s
      ...
      || h.cid.toLowerCase().includes(s)
      || h.date.includes(s);
  });

  return (
    <div>
      {/* Aviso quando a persistência não está operando */}
      {storagePersisted === false && (
        <div style={{
          ...sCard(t), padding: 14, marginBottom: 12,
          border: `1px dashed ${t.warning}`,
          display: 'flex', alignItems: 'center', gap: 8,
          fontSize: 13, color: t.warning,
        }}>
          <AlertTriangle size={16} />
          Armazenamento local indisponível — os dados desta sessão não serão salvos.
        </div>
      )}

      {/* Barra de busca e ações */}
      <div style={{
        ...sCard(t), display: 'flex', justifyContent: 'space-between',
        alignItems: 'center', flexWrap: 'wrap', gap: 12, padding: 16,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 200 }}>
          <Search size={18} style={{ color: t.textMuted }} />
          <input
            style={{ ...sInput(t), maxWidth: 350 }}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar por nome, CID ou data..."
          />
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
        ...
        {filtered.slice(0, 60).map((h) => {
          ...
          return (
            <div key={h.id} style={{
              ...sCard(t), padding: 14, display: 'flex',
              justifyContent: 'space-between', alignItems: 'center',
              borderLeft: '4px solid ' + sc.bg, marginBottom: 0,
            }}>
              <div style={{ flex: 1 }}>
                <div style={{
                  fontWeight: 700, color: t.text, fontSize: 15,
                  display: 'flex', alignItems: 'center', gap: 8,
                }}>
                  {h.name}
                  {isJudicial && (
                    <span style={{
                      ...
                      fontWeight: 700, border: '1px solid #7c3aed33',
                    }}>
                      VIA JUDICIAL
                    </span>
                  )}
                ...
                <div style={{
                  fontSize: 13, color: t.textMuted, display: 'flex',
                  flexWrap: 'wrap', gap: 8, marginTop: 2,
                }}>
